...
- **`master` is a protected branch.** Changes go through a pull request; PRs must be up to date with `master` (strict status checks — `allow_update_branch` is on, so use GitHub's "Update branch" button, or `git merge origin/master` into the PR branch, rather than assuming it'll resolve itself). `.github/workflows/ci.yml` (ruff + pytest) must pass before merging. No required review (solo-dev project). Repo is squash-merge-only (`allow_merge_commit`/`allow_rebase_merge` off) with `delete_branch_on_merge` on, matching `required_linear_history`.
...
- **The exe build is a second job in the same `release-please.yml` workflow** (`build`, `needs: release-please`, gated on `needs.release-please.outputs.release_created == 'true'`), not a separate `on: release published` workflow. That was tried first and never fired: `release-please-action` creates the release using `GITHUB_TOKEN`, and GITHUB_TOKEN-triggered events don't cascade to trigger *other* workflow files (an anti-recursion measure) — job outputs within one workflow run aren't subject to that. The `build` job checks out the exact tag, builds `KeyHolder.exe` with PyInstaller on `windows-latest`, and uploads it via `gh release upload`. The workflow also accepts manual `workflow_dispatch` with a `tag` input, to (re)build and attach the exe for an existing release without needing a new version bump — this is how v1.0.0's asset was backfilled after the cascade issue was discovered and fixed post-release.
